...
import {CreateSubscriptionPayload, CreateSubscriptionResponse, SubscriptionDetails, TYPES} from "@/types";
...
export class SubscriptionService implements ISubscriptionService {
    ...
    async createSubscription(payload: CreateSubscriptionPayload): Promise<CreateSubscriptionResponse> {
        ...
        const activeSub = await this._subscriptionRepository.findActiveByUserId(payload.userId);
        if (activeSub) {
    ...
    async getSubscriptionDetails(subscriptionId: string): Promise<SubscriptionDetails | null> {
    ...
    async getActiveSubscriptionByUserId(userId: string): Promise<SubscriptionDetails | null> {
    ...
    async cancelSubscription(subscriptionId: string, cancelAtPeriodEnd = true): Promise<SubscriptionDetails | null> {
        const sub = await this._subscriptionRepository.findById(subscriptionId);
        if (!sub) return null;
        ...
        const updatedSub = await this._subscriptionRepository.update(subscriptionId, sub);

...
export function subscriptionRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/subscriptions', async (request, reply) => {
    try {
      const { planId, userId } = request.body as {
        planId: string;
        userId: string;
      };
  ...
  fastify.get('/subscriptions/:id', async (request, reply) => {
    ...
    try {
      const subscription =
        await subscriptionController.getActiveSubscription(id);
      if (!subscription) {
      ...
      return reply
        .status(500)
        .send({ error: 'Failed to retrieve subscription' });
    }
  });
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      try {
        const subscriptions =
          await subscriptionController.listUserSubscriptions(userId);
        return reply.send(subscriptions);
      } catch (error) {
        request.log.error(error);
        return reply
          .status(500)
          .send({ error: 'Failed to list subscriptions' });
      }
    },
  );
}

...
export default function ChatWindow({chat, isNewChat}: ChatWindowProps) {
    ...
    useEffect(() => {
        console.log('trigger fetch messages');
    ...
    useEffect(() => {
        console.log('trigger set chat messages');
        if (!chat) return;
    ...
    useEffect(() => {
        console.log('trigger mark as read');
        const lastMessage = chatMessages.at(-1);
        if (lastMessage && lastMessage.sender !== user?.id) {
            markAsRead(chat.id);
        }
    }, [user?.id, chat.id, markAsRead]);

    useEffect(() => {
        console.log('trigger scroll to bottom');
    ...
    useEffect(() => {
        console.log('trigger adjust scroll on prepend');
    ...
    useEffect(() => {
        console.log('trigger typing indicator');
        if (!chat) return;
